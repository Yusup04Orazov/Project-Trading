...
def fit_arima_and_predict(prices):
    ...
    auto_arima_results = pm.auto_arima(log_prices, seasonal=False, trace=True, error_action='ignore', suppress_warnings=True, stepwise=True, stationary=False, d=None)
...
def generate_buy_signal(symbol, current_price, macd, signal_line):
    ...
    if check_total_profit_threshold():
        ...
        current_price*0.99 > data[symbol]['moving_average_200'][-1] and 
...
def generate_sell_signal(symbol, current_price, macd, signal_line):
    ...
    if data[symbol]['last_trade_action'] == "buy":
        ...
        if recent_buy_signal:
            ...
            if (rsi > 60 and macd > 0 and signal_line > 0 and macd < signal_line) or (current_price >= (1.003 * recent_buy_signal['bought_price'])) or (current_price <= 0.984 * recent_buy_signal['bought_price']):
...
while True:
    for symbol in symbols:
        ...
        # # Visualize trade data from trade.jsonl
        # if os.path.exists(buy_sell_signals_file):
        #     with open(buy_sell_signals_file, 'r') as trade_file:
        #         trade_data = [json.loads(line.strip()) for line in trade_file]

        #     symbols_traded = set([trade['symbol'] for trade in trade_data])
            
        #     for symbol_traded in symbols_traded:
        #         symbol_trades = [trade for trade in trade_data if trade['symbol'] == symbol_traded]
        #         trade_actions = [trade['action'] for trade in symbol_trades]
        #         portfolio_values = [trade['portfolio'] for trade in symbol_trades]

        #         plt.figure()
        #         plt.plot(range(len(symbol_trades)), portfolio_values, label='Portfolio Value', marker='o', color='blue')
        #         plt.xlabel('Trade Number')
        #         plt.ylabel('Portfolio Value ($)')
        #         plt.title(f'Portfolio Value for {symbol_traded}')
        #         plt.legend()
        #         plt.savefig(f'{symbol_traded}_portfolio.png')